...
def load_vectors(filename='glove.6B.100d.txt'):
    ...
    if 'emoji' in filename:
        f = open(path + '/models/emoji2vec/' + filename, encoding = 'utf-8')
    else:   # by default, load the GLoVe embeddings
        f = open(path + '/res/glove/' + filename, encoding = 'utf-8')
    for line in f:
...
def plot_training_statistics(history, plot_name, also_plot_validation=False, acc_mode='acc', loss_mode='loss'):
    ...
    print("Plot for accuracy saved to %s" % (plot_name + "_acc.png"))
    ...
    print("Plot for loss saved to %s" % (plot_name + "_loss.png"))
